...
def read_query_results(query_file_name):
    return pd.read_csv("expected-output/" + query_file_name + ".csv")
...
def run_tests():
    hubmap = hubmapy.HuBMAPy(output_folder="current-output/")
    test_biomarkers_for_all_cell_types(query_engine=hubmap)
...
def do_all_queries():
    hubmap = hubmapy.HuBMAPy(output_folder="expected-output/")
